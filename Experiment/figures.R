...
performances_XOR_imbalance_summary <- 
  prepare_df_for_figures_simulation(performances_XOR_imbalance_0.1_0.2_ACCheaviside[,-c(1,8)]) %>% 
...
for(alg_name in unique(performances_XOR_imbalance_summary$algorithm)){
  assign(paste0("g_", alg_name), performances_XOR_imbalance_summary %>%
           ...
           scale_color_discrete(breaks = c("A_index", "C_index_d", "C_index_t",
                                           "C_index", "accuracy"),
                                ...
                                type = c("#F8766D", "#7CAE00", "#00BFC4", "#C77CFF",
                                         "#999999")) +
           scale_fill_discrete(breaks = c("A_index", "C_index_d", "C_index_t",
                                          "C_index", "accuracy"),
                               labels = c("IC-index", bquote(C[d]-index), 
                                          bquote(C[t]-index),
                                          "C-index", "Accuracy"),
