# Running the original [GraphDTA](https://github.com/thinng/GraphDTA) experiment:
...
The `.sh` files in the repository can be used as they are on CSC to reproduce the results. Further info on using such files on CSC in the [Feedforward repository](https://gitlab.utu.fi/soheik/drug-target-feedforward)

The `create_cv_data.py` code must first be run. It produces `.pth` files that are used by PyTorch - the files are a little bloated as they don't cleverly use indexing, but it shouldn't be a big problem. 
